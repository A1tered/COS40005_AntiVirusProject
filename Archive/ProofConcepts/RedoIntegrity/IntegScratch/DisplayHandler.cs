...
{
    // Class is primarily responsible for building displayable tables, as C# lacks it fundamentally.
    public static class DisplayHandler
    {

        public static string ByteToSize(long byteAmount)
        {
            string[] sizeSuffix = {"B","KB", "MB", "GB", "TB"};
            long conversionAmount = byteAmount;
            int suffixIndex = 0;
            while (conversionAmount / 1024 > 1)
            {
                conversionAmount = conversionAmount / 1024;
                suffixIndex++;
            }
            return $"{conversionAmount}{sizeSuffix[suffixIndex]}";
        }

        // For a list collection, with the first row being considered the field names, produces a table like display.
