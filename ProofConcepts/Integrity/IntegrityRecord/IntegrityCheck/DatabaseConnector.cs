...
namespace FindTheHash
{
    public class DatabaseConnector
    {
        ...
        public Tuple<string, long, long> QueryDirectoryData(string directory)
        {
            if (ConnectionSuccessful())
            {
                ...
                if (sqliteDataReader.HasRows)
                {
                    ...
                    return new Tuple<string, long, long>(hash, modificationTime, signatureTime);
                }
            }
            return new Tuple<string, long, long>("", 0, 0);
        }
        ...
        public bool AddIntegrityEntry(string directory, string hash, long modificationTime, long signatureTime)
        {
            if (ConnectionSuccessful())
            {
                Tuple<string, long, long> result = QueryDirectoryData(directory);
                SqliteCommand commandCreation = _sqliteConnectionRepresentation.CreateCommand();
                if (result.Item1 == "") // No existent entry
                {
                    Console.WriteLine("No record, adding new record.");
                    commandCreation.CommandText = (@$"INSERT INTO {_tableName} VALUES ($directory, $hash, $modificationTime, $signatureTime) ;
                ");
                }
                else // Update existing value
                {
                    Console.WriteLine("Existent entry, updating entry.");
                    commandCreation.CommandText = (@$"UPDATE {_tableName} SET directory = $directory, hash = $hash, modificationTime = $modificationTime, signatureTime = $signatureTime WHERE hash=$replaceHash And signatureTime=$replaceSignatureTime ;");
